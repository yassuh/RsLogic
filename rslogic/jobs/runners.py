...
class RsToolsRunner:
    ...
    def run(self, working_directory: Path, image_keys: Sequence[str], filters: Dict[str, Any]) -> Dict[str, Any]:
...
class RsToolsSdkRunner(RsToolsRunner):
    ...
    def _wait_for_task(self, client: Any, task_id: str, *, timeout_seconds: int) -> Dict[str, Any]:
        ...
        while True:
            ...
            if statuses:
                ...
                if state in self._FAIL_STATES or int(status.errorCode) != 0:
                    raise RuntimeError(
                        f"RealityScan task failed task_id={task_id} state={status.state} "
                        f"error_code={status.errorCode} error_message={status.errorMessage}"
                    )
    ...
    def run(self, working_directory: Path, image_keys: Sequence[str], filters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with RealityScanClient(  # type: ignore[misc]
            ...
            # Mirrors the command sequence from the working notebook flow.
            executed_commands.append(
                self._run_command(client, command_name="newScene", params=None, timeout_seconds=task_timeout_seconds)
            )
            executed_commands.append(
                self._run_command(
                    client,
                    command_name="set",
                    params=[f"appIncSubdirs={'true' if include_subdirs else 'false'}"],
            ...
            executed_commands.append(
                self._run_command(
                    client,
                    command_name="set",
                    params=[f"sfmCameraPriorAccuracyX={camera_prior_xyz}"],
                    timeout_seconds=task_timeout_seconds,
                )
            )
            executed_commands.append(
                self._run_command(
                    client,
                    command_name="set",
                    params=[f"sfmCameraPriorAccuracyY={camera_prior_xyz}"],
                    timeout_seconds=task_timeout_seconds,
                )
            )
            executed_commands.append(
                self._run_command(
                    client,
                    command_name="set",
                    params=[f"sfmCameraPriorAccuracyZ={camera_prior_xyz}"],
                    timeout_seconds=task_timeout_seconds,
                )
            )
            executed_commands.append(
                self._run_command(
                    client,
                    command_name="set",
                    params=[f"sfmCameraPriorAccuracyYaw={camera_prior_ypr}"],
                    timeout_seconds=task_timeout_seconds,
                )
            )
            executed_commands.append(
                self._run_command(
                    client,
                    command_name="set",
                    params=[f"sfmCameraPriorAccuracyPitch={camera_prior_ypr}"],
                    timeout_seconds=task_timeout_seconds,
                )
            )
            executed_commands.append(
                self._run_command(
                    client,
                    command_name="set",
                    params=[f"sfmCameraPriorAccuracyRoll={camera_prior_ypr}"],
                    timeout_seconds=task_timeout_seconds,
                )
            )
            executed_commands.append(
                self._run_command(
                    client,
                    command_name="set",
                    params=[f"sfmDetectorSensitivity={detector_sensitivity}"],
                    timeout_seconds=task_timeout_seconds,
                )
            )
            executed_commands.append(
                self._run_command(
                    client,
                    command_name="addFolder",
                    params=[imagery_folder],
                    timeout_seconds=task_timeout_seconds,
                )
            )
            ...
            if run_normal_model:
                executed_commands.append(
                    self._run_command(
                        client,
                        command_name="calculateNormalModel",
                        params=None,
                        timeout_seconds=task_timeout_seconds,
                    )
                )
            if run_ortho_projection:
                executed_commands.append(
                    self._run_command(
                        client,
                        command_name="calculateOrthoProjection",
                        params=None,
                        timeout_seconds=task_timeout_seconds,
                    )
                )
